...
纯前端暗房工具（React + TypeScript + Vite），顶部入口切换两个互不影响的功能：
...
- **创建批次**：填写药液名称与额定容量（整批可处理的等效胶片总数，正整数）；
  空名称、容量留空 / 非整数 / 非正整数会在对应字段下方就地说明原因，不创建批次。
- **登记用量**：选中批次后登记本次处理的等效胶片数量与备注（可选）；
  每条记录写入前都重新计算剩余量（额定容量 − 已登记用量之和），
  数量留空 / 非整数 / 非正整数 / 超过剩余容量时就地说明原因，不写入记录。
...
src/lib/ledgerStorage.ts   台账的 localStorage 持久化（读取时逐字段校验结构）
src/App.tsx                顶部功能切换、配液表单、结果、量取步骤、打印配液卡
src/Ledger.tsx             容量台账视图：新建批次、批次列表、登记用量、使用记录
